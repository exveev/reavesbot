      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "32px 24px" }} className="fade-up">
        ...
        <div style={{ background: C.surface, borderRadius: 16, padding: 24, border: `1px solid ${C.border}`, marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {/* Check result */}
            {checkResult && (
              <div style={{ marginTop: 10, padding: "10px 14px", borderRadius: 8, background: checkResult.available ? "#081a08" : "#1a0808", border: `1px solid ${checkResult.available ? "#155a15" : "#4a1515"}` }}>
                <p style={{ color: checkResult.available ? C.green : C.accent, fontFamily: ff.b, fontSize: 13 }}>
                  {checkResult.available ? `✓ @${username} is available!` : `✗ @${username} is taken`}
                </p>
              </div>
            )}
          </div>
        ...
        {!running && logs.length === 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 8 }}>
            {[
              ...
              { emoji: "🔒", title: "Your creds", desc: "Credentials never stored — session only" },
            ].map(c => (
